...
(function ($) {
    ...
    DApiUI.createDescription=function (menu) {
        var table=$('<table class="table table-hover table-bordered table-text-center" style="font-size: 12px;"></table>');
        var thead=$('<thead><tr><th colspan="2" style="text-align:center">微服务框架api接口文档</th></tr></thead>');
        table.append(thead);
    ...
    DApiUI.createDebugTab=function (apiInfo) {
        ...
        if(typeof (apiInfo.parameters)!='undefined'&&apiInfo.parameters!=null){
            ...
            $.each(apiInfo.parameters,function (i, param) {
                ...
                if (param.in == "body"){
                    var val=DApiUI.getStringValue(param['description']);
                        ...
                        if(regex.test(ref)) {
                            ...
                            for(var i=0;i<definitionsArray.length;i++){
                                ...
                                if(definition.key==refType){
                                    if (paramArray != null){
                                        paramArray.push(definition.value);
                                        val = JSON.stringify(paramArray);
                                    }else {
                                        val = JSON.stringify(definition.value);
                                    }
                                }
                            }
                        }
                    }
                    value=$('<td><textarea class="form-control p-value" data-apiUrl="'+apiInfo.url+'" data-name="'+param.name+'" rows="5">' + val +'</textarea></td>');
                }
                var oper=$('<td><button class="button border-blue" type="button">删除</button></td>');
                //删除事件
            ...
            $.ajax({
                ...
                success:function (data,status,xhr) {
                    ...
                    if(allheaders!=null&&typeof (allheaders)!='undefined'&&allheaders!=""){
                        ...
                        var headertable=$('<table class="table table-hover table-bordered table-text-center" style="font-size: 12px;"><tr><th>请求头</th><th>value</th></tr></table>');
    ...
    DApiUI.createApiInfoTable=function (apiInfo) {
        ...
        if(typeof (apiInfo.parameters)!='undefined'&&apiInfo.parameters!=null){
            var ptd=$("<td></td>");
            var ptable=$('<table class="table table-bordered" style="font-size: 12px;"></table>')
            var phead=$('<thead><th>参数名称</th><th>说明</th><th>类型</th><th>in</th><th>是否必须</th><th>对象说明</th></thead>');
            ptable.append(phead);
                    ...
                    if(param.hasOwnProperty("schema")){
                        ...
                        if(schema.hasOwnProperty("type")){
                            ...
                            if(regex.test(ref)) {
                                ...
                                for(var i=0;i<definitionsArray.length;i++){
                                    var definition=definitionsArray[i];
                                    if(definition.key==ptype){
                                        // div = "<div style='width:220px;'>" + JSON.stringify(definition.model).replace(/(\}\,)/g, "$1<br/>&nbsp;") + "</div>";
                                        div.html("")
                                        div.JSONView(definition.model);
                                    }
                                }
                            }
                        }
            ...
            for(var definition in definitions){
                ...
                if (checkUndefined(value)){
                    ...
                    if(value.hasOwnProperty("properties")){
                        ...
                        defiType.model = value["properties"];

                        var defiTypeValue={};
                        for(var property in properties){
                            var propobj=properties[property];
                            for (var i = 0; value.required && i < value.required.length; i++){
                                if (value.required[i] == property){
